...
class SdkArtifactTests(unittest.TestCase):
    ...
    def test_reproducible_archive_installs_into_sterile_consumer(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(verification["framework_version"], "0.1.7")
            self.assertEqual(verification["cli_version"], "0.1.7")
            ...
            self.assertEqual(validation["cli_version"], "0.1.7")
            ...
            if shutil.which("curl"):
                release_dir = root / "release/v0.1.7"
                ...
                release_install = self.run_checked([
                    "sh", str(INSTALLER), "--version", "0.1.7",
                    "--target", "test-host",
