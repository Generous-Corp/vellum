...
class CliTests(unittest.TestCase):
    def test_version_matches_the_immutable_framework_release(self) -> None:
        ...
        self.assertEqual(completed.stdout.strip(), "vellum 0.1.5")
    ...
    def test_create_from_composes_initial_import_with_permanent_source_key(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (sdk / "metadata.json").write_text(json.dumps({
                "schema": "vellum.sdk-artifact.v1", "framework_version": "0.1.5",
                "cli_version": "0.1.5", "cli_api": 1, "source_commit": None,
                "target": "local-development",
            ...
            (sdk / "install-manifest.json").write_text(json.dumps({
                ...
                "artifact": None, "artifact_sha256": None, "framework_version": "0.1.5",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                ...
                [
                    ...
                    "--framework-version", "0.1.5", "--cli-api", "1",
                    "--target", "web",
                ],
            )
    ...
    def test_installed_ui_projection_satisfies_exact_npm_lock(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (sdk / "install-manifest.json").write_text(json.dumps({
                ...
                "framework_version": "0.1.5", "target": "local-development",
                "source_commit": None,
            }), encoding="utf-8")
            project = root / "app"
            created = invoke(
    ...
    def test_project_pins_and_enforces_exact_installed_artifact_sha(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest = {
                "schema": "vellum.sdk-install.v1",
                "verified": True,
                "artifact": "vellum-sdk-0.1.5-test-host.tar.gz",
                "artifact_sha256": "b" * 64,
                "framework_version": "0.1.5",
                "target": "test-host",
                "source_commit": "a" * 40,
