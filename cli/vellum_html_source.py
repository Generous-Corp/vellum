...
def _split_references(value: str, attribute: str) -> list[str]:
    if attribute == "srcset":
        return [item.strip().split(None, 1)[0] for item in value.split(",") if item.strip()]
...
def discover_dependencies(source: Path, fingerprint: dict[str, object]) -> list[Dependency]:
    ...
    for item in references:
        ...
        if relative not in dependencies:
            dependencies[relative] = Dependency(
                path=relative, kind=_dependency_kind(str(item.get("tag", "")), relative),
    ...
    for dependency in list(dependencies.values()):
        if dependency.kind != "stylesheet":
            continue
        css = (root / dependency.path).read_text(encoding="utf-8", errors="strict")
        for reference in CSS_URL_RE.findall(css):
            relative = _relative_reference(reference)
            if relative is None or relative in dependencies:
                continue
            path = _regular_file(root, relative)
            data = path.read_bytes()
            if len(data) > MAX_DEPENDENCY_BYTES:
                raise HTMLSourceError(f"HTML dependency exceeds {MAX_DEPENDENCY_BYTES} bytes: {relative!r}")
            dependencies[relative] = Dependency(
                path=relative, kind=_dependency_kind("", relative),
                bytes=len(data), sha256=sha256(data).hexdigest())
            total += len(data)
            if len(dependencies) > MAX_DEPENDENCIES or total > MAX_TOTAL_BYTES:
                raise HTMLSourceError("HTML source dependency graph exceeds its bounded budget")
