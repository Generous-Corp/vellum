...
The following is the exact `v0.1.5` tagged-release flow; it does not claim that
...
gh release download v0.1.5 \
  ...
  --pattern install.sh \
  --dir "$bootstrap_dir"
gh release verify-asset v0.1.5 "$bootstrap_dir/install.sh" \
  --repo Generous-Corp/vellum
sh "$bootstrap_dir/install.sh" --version 0.1.5
...
do
  gh release verify-asset v0.1.5 "$bootstrap_dir/$asset" \
    --repo Generous-Corp/vellum
done
(
  cd "$bootstrap_dir"
...
./scripts/install.sh \
  --archive dist/vellum-sdk-0.1.5-darwin-arm64.tar.gz \
...
The exact `v0.1.5` release is consumed without a moving `latest` pointer:

<!-- readme-exec: id=version-install-only manual=subset-of-release-quick-start -->
```sh
./scripts/install.sh --version 0.1.5
```
...
```sh
gh release verify v0.1.5 --repo Generous-Corp/vellum
gh release verify-asset v0.1.5 ./vellum-sdk-0.1.5-darwin-arm64.tar.gz \
  --repo Generous-Corp/vellum
```
