...
// Named post-parse rules. The heuristic geometry rewrites (separator
// promotion, rounded-corner inheritance, shadow-driven sibling snap,
// connector-line spanning) live in normalize_design_ir()
// (design_ir_normalize.cpp); the widget-semantics passes stay here.
...
IRNode parse_ir_node(const choc::value::ValueView& obj) {
    ...
    // Heuristic geometry rewrites — separator promotion, rounded-corner
    // inheritance, shadow-driven sibling snap, and the connector-line
    // spanning rule — extracted to normalize_design_ir()
    // (design_ir_normalize.cpp). Runs after children and alternate frames
    // are parsed so every rule sees final child geometry; children were
    // normalized during their own parse_ir_node recursion.
